...
describe('dashboard', () => {
  ...
  it('switches the full interface to Kazakh from the language menu', async () => {
    render(<App />);
    await screen.findByText('Power forecast');
    fireEvent.click(screen.getByRole('button', { name: 'Language' }));
    fireEvent.click(screen.getByRole('menuitem', { name: /Қазақша/i }));
    expect(screen.getByRole('link', { name: 'Бақылау тақтасы' })).toHaveClass('active');
    expect(screen.getByRole('button', { name: /Болжамды іске қосу/i })).toBeInTheDocument();
    expect(await screen.findByText('Қуат болжамы')).toBeInTheDocument();
    expect(localStorage.getItem('wind-language')).toBe('kk');
  });
});
