...
Реализован первый вертикальный сценарий: точечная загрузка архивного NOAA GFS, строгие временные проверки и воспроизводимый audit погодных входов. Модель мощности и приложение ещё не реализованы, поэтому сценарий не выдаёт численный прогноз.
...
Для загрузки из архива NOAA установите `requirements-weather.txt` и уберите параметр `--snapshot`. Результат подтверждает готовность погодных входов, но не готовность прогноза мощности.
